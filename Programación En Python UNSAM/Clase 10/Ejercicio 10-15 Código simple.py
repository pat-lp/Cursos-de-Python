...
def parsear_datos(lines):
    ...
    rows =( (func(val) for func,val in zip([str, float, int],row))for row in rows) #cambiar_tipo
...
def ticker(camion_file, log_file, fmt):
    ...
    for fila in rows:
        formateador.fila(str(s) for s in fila.values())

    

if __name__ == '__main__':
    ticker('../Data/camion.csv', '../Data/mercadolog2.csv', 'csv')
